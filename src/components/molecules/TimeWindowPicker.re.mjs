...
function groupCourtAvailabilityByTime(courtAvailability) {
  ...
  courtAvailability.forEach(function (court) {
        court.intents.forEach(function (intent) {
              var key = intent.start.toString() + ":" + intent.end.toString();
              var group = Js_dict.get(groups, key);
              if (group !== undefined) {
                groups[key] = {
                  key: group.key,
                  start: group.start,
                  end: group.end,
                  slots: Belt_Array.concat(group.slots, [{
                          court: court,
                          intent: intent
                        }])
                };
  ...
  return Js_dict.values(groups).toSorted(function (a, b) {
              if (a.start === b.start) {
                return a.end - b.end;
              } else {
...
function TimeWindowPicker(props) {
  var onUseCourtSlot = props.onUseCourtSlot;
  ...
  var courtGroups = groupCourtAvailabilityByTime(courtAvailability);
  return JsxRuntime.jsxs("div", {
              children: [
                ...
                JsxRuntime.jsxs("div", {
                      children: [
                        ...
                        courtGroups.map(function (group) {
                              var leftPct = (group.start - hourMinVal) / hourRangeVal * 100.0;
                              var widthPct = (group.end - group.start) / hourRangeVal * 100.0;
                              var canMatch = group.end - group.start <= 4.0;
                              ...
                              return JsxRuntime.jsx("button", {
                                          children: JsxRuntime.jsx("span", {
                                                children: group.slots.length.toString(),
                                                className: "font-mono text-[8px] font-bold leading-none text-cyan-950 dark:text-cyan-100 pointer-events-none"
                                              }),
                                          className: "absolute top-1 h-3.5 min-w-5 rounded-sm border border-cyan-600/70 bg-cyan-300/80 dark:bg-cyan-500/45 z-[15] flex items-center justify-center px-1 " + (
                                            canMatch && Core__Option.isSome(onUseCourtSlot) ? "cursor-pointer hover:bg-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-inset" : "pointer-events-none"
                                          ),
                                          style: {
                                            left: leftPct.toString() + "%",
                                            width: widthPct.toString() + "%"
                                          },
                                          title: courtsPhrase + " · " + hourLabel(group.start) + "–" + hourLabel(group.end) + (
                                            canMatch ? " · " + t`Match my time` : " · " + t`Long opening`
                                          ),
                                          disabled: !canMatch || Core__Option.isNone(onUseCourtSlot),
                                          type: "button",
                                          onClick: (function (e) {
                                              e.stopPropagation();
                                              if (canMatch && onUseCourtSlot !== undefined) {
                                                return onUseCourtSlot(group);
                                              }
