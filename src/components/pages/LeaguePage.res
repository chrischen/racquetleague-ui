              ...
              <dl className="max-w-xl space-y-8 text-base leading-7 text-gray-300 lg:max-w-none">
                  <div className="relative">
                    <dt className="ml-9 inline-block font-semibold text-white">
                      <CloudArrowUpIcon className="absolute left-1 top-1 h-5 w-5 text-indigo-500" \"aria-hidden"="true" />
                      {t`Hesacore Grip`}
                    </dt>{" "->React.string}
    ...
    {() => {
      <>
        ...
        <div className="py-10 text-white bg-leaguePrimary">
          <Layout.Container>
            <PageTitle> <span className="text-white font-extrabold text-3xl">{t`Recreational Doubles Rankings`}</span> </PageTitle>
            <p className="mt-5"> {t`Play doubles games with different partners and receive an individual rating. Prizes are awarded monthly to top players.`} </p>
            <p> {t`Currently the league is in testing mode. To participate, please join the Pickleball events here:`}{" "->React.string}<a className="text-gray-200" href="https://www.racquetleague.com">{"> Racquet League <"->React.string}</a></p>
          </Layout.Container>
        </div>
